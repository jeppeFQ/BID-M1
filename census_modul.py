...
def tilføj_person_interaktivt(personer, husstande, institutioner, arbejdspladser):
    ...
    # Validering af om personen er student eller arbejder
    while True:
        type_person = input("Er personen en Student eller Arbejder? (S/A): ").strip().upper()
        if type_person in ["S", "A"]:
            break
        else:
            print("Ugyldigt valg. Indtast 'S' for Student eller 'A' for Arbejder.")
...
def main():
    ...
    while True:
        ...
        if valg == "1":
            ...
            klasse_str = input("Skriv klasse (Student/Arbejder): ")
